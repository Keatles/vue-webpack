...
<template>
  <header>
    ...
    <ul class="nav" role="navigation">
      <li v-for="(item,index) in data">
        ...
        <ul v-if="item.childrens" class="tab">
          <li v-for="(obj,i) in item.childrens" :class="{ 'separate':obj.separate }">
    ...
    <ul v-if="isSmallScreen" class="mobile-menu">
      ...
      <li v-for="(item,index) in data" :class="{ 'separate': item.seperate,'right-arrow' : item.canClick }" @click="switchMenu($event, item)">
...
<! -- use 'scoped' to limit css to this component only -->
...
.mobile-menu {
  ...
  @include min-screen($mobile-width) {
    display: none;
  }
}
</style>
